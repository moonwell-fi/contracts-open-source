        ...
        if (eventArg === undefined) {
          ...
          try {
            ...
            if (arg.rescue) {
              ...
              // `Gate (QiToken Eth Address) (... deploy qiToken)`
              // could be used to deploy a qiToken if it doesn't exist, but
              // since there is no QiToken, that check would raise (when we'd
