...
import { qiTokenCommands, processQiTokenEvent } from './Event/QiTokenEvent';
import { qiTokenDelegateCommands, processQiTokenDelegateEvent } from './Event/QiTokenDelegateEvent';
...
import { qiCommands, processQiEvent } from './Event/QiEvent';
...
import { BenqiLens } from './Contract/BenqiLens';
...
async function sendAvax(world: World, from: string, to: string, amount: encodedNumber): Promise<World> {
...
export const commands: (View<any> | ((world: World) => Promise<View<any>>))[] = [
  ...
  new Command<{ blockNumber: NumberV, event: EventV }>(
    `
      ...
      * "Block 10 (...event)" - Set block to block N and run event
        * E.g. "Block 10 (Benqi Deploy Admin)"
    `,
    'Block',
    [
      new Arg('blockNumber', getNumberV),
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "Trx ...trxEvent" - Handles event to set details of next transaction
        * E.g. "Trx Value 1.0e18 (QiToken qiAvax Mint 1.0e18)"
    `,
    'Trx',
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "Expect ...expectation" - Adds an expectation to hold after the next transaction
        * E.g. "Expect Changes (QiToken qiZRX TotalSupply) +10.0e18"
    `,
    'Expect',
    [new Arg('event', getEventV, { variadic: true })],
  ...
  new Command<{ given: Value; event: EventV }>(
    `
      ...
      * "Given value event" - Runs event only if value is truthy. Thus, given can be used to build existence checks.
        * E.g. "Given ($var) (PriceOracle SetPrice qiBAT $var)"
    `,
    'Given',
  ...
  new Command<{ address: AddressV; amount: NumberV }>(
    `
      ...
      * "Send <Address> <Amount>" - Sends a given amount of eth to given address
        * E.g. "Send qiAVAX 0.5e18"
    `,
    'Send',
    [new Arg('address', getAddressV), new Arg('amount', getNumberV)],
    (world, from, { address, amount }) => sendAvax(world, from, address.val, amount.encode())
  ),
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "QiToken ...event" - Runs given QiToken event
        * E.g. "QiToken qiZRX Mint 5e18"
    `,
    'QiToken',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => processQiTokenEvent(world, event.val, from),
    { subExpressions: qiTokenCommands() }
  ),
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "QiTokenDelegate ...event" - Runs given QiTokenDelegate event
        * E.g. "QiTokenDelegate Deploy QiDaiDelegate qiDaiDelegate"
    `,
    'QiTokenDelegate',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => processQiTokenDelegateEvent(world, event.val, from),
    { subExpressions: qiTokenDelegateCommands() }
  ),
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * E.g. "PriceOracleProxy Deploy (Unitroller Address) (PriceOracle Address) (QiToken qiAVAX Address)"
    `,
    'PriceOracleProxy',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * E.g. "Maximillion Deploy (QiToken qiAVAX Address)"
    `,
    'Maximillion',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
  ...
  new Command<{ event: EventV }>(
    `
      ...
      * "Benqi ...event" - Runs given benqi event
      * E.g. "Benqi Deploy"
    `,
    'Benqi',
    [new Arg('event', getEventV, { variadic: true })],
    (world, from, { event }) => {
      return processQiEvent(world, event.val, from);
    },
    { subExpressions: qiCommands() }
  ),
  ...
  buildContractEvent<BenqiLens>("BenqiLens", false),
