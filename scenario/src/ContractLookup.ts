...
export function getQiTokenAddress(world: World, qiTokenArg: string): string {
  return getContractDataString(world, [['qiTokens', qiTokenArg, 'address']]);
}

export function getQiTokenDelegateAddress(world: World, qiTokenDelegateArg: string): string {
  return getContractDataString(world, [['QiTokenDelegate', qiTokenDelegateArg, 'address']]);
}
...
export async function getComp(
  ...
  return getWorldContract(world, [['Benqi', 'address']]);
}

export async function getQiData(
  world: World,
  compArg: string
): Promise<[Benqi, string, Map<string, string>]> {
  let contract = await getComp(world, <Event>(<any>compArg));
  let data = getContractData(world, [['Benqi', compArg]]);
...
export async function getQiTokenData(
  world: World,
  qiTokenArg: string
): Promise<[QiToken, string, Map<string, string>]> {
  let contract = getWorldContract<QiToken>(world, [['qiTokens', qiTokenArg, 'address']]);
  let data = getContractData(world, [['QiTokens', qiTokenArg]]);

  return [contract, qiTokenArg, <Map<string, string>>(<any>data)];
}

export async function getQiTokenDelegateData(
  world: World,
  qiTokenDelegateArg: string
): Promise<[QiErc20Delegate, string, Map<string, string>]> {
  let contract = getWorldContract<QiErc20Delegate>(world, [['QiTokenDelegate', qiTokenDelegateArg, 'address']]);
  let data = getContractData(world, [['QiTokenDelegate', qiTokenDelegateArg]]);

  return [contract, qiTokenDelegateArg, <Map<string, string>>(<any>data)];
}
...
export function getAddress(world: World, addressArg: string): string {
  ...
  return getContractDataString(world, [
    ...
    ['qiTokens', addressArg, 'address'],
    ['QiTokenDelegate', addressArg, 'address'],
