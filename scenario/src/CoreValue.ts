...
import { qiTokenFetchers, getQiTokenValue } from './Value/QiTokenValue';
import { qiTokenDelegateFetchers, getQiTokenDelegateValue } from './Value/QiTokenDelegateValue';
...
import { getQiValue, qiFetchers } from './Value/QiValue';
...
async function getAvaxBalance(world: World, address: string): Promise<NumberV> {
...
const fetchers = [
    ...
    async (world, { addr, slot, key, nestedKey, valType }) => {
      ...
      switch (valType.val) {
        case 'marketStruct':
          ...
          let isQiKey = '0x' + toBN(newKey).add(toBN(3)).toString(16);
          let isQiStr = await world.web3.eth.getStorageAt(addr.val, isQiKey);

          return new ListV([
            new BoolV(isListed),
            new ExpNumberV(collateralFactor.toString(), 1e18),
            new BoolV(areEqual(userInMarket, 1)),
            new BoolV(areEqual(isQiStr, 1))
          ]);
        default:
          return new NothingV();
      }
  ...
  new Fetcher<{ address: AddressV }, Value>(
    `
      ...
      * "AvaxBalance <Address>" - Returns given address' avax balance.
    `,
    'AvaxBalance',
    [new Arg('address', getAddressV)],
    (world, { address }) => getAvaxBalance(world, address.val)
  ),
  new Fetcher<{ given: Value; expected: Value }, BoolV>(
    `
      ...
      * "Equal given:<Value> expected:<Value>" - Returns true if given values are equal
        ...
        * E.g. "Equal (QiToken qiZRX TotalSupply) (Exactly 55)"
        * E.g. "Equal (QiToken qiZRX Comptroller) (Comptroller Address)"
    `,
    'Equal',
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "QiToken ...qiTokenArgs" - Returns qiToken value
    `,
    'QiToken',
    [new Arg('res', getQiTokenValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: qiTokenFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### QiTokenDelegate

      * "QiTokenDelegate ...qiTokenDelegateArgs" - Returns qiToken delegate value
    `,
    'QiTokenDelegate',
    [new Arg('res', getQiTokenDelegateValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: qiTokenDelegateFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### Erc20
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "Benqi ...compArgs" - Returns Benqi value
    `,
    'Benqi',
    [new Arg('res', getQiValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: qiFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### Governor
...
let contractFetchers = [
  ...
  { contract: "BenqiLens", implicit: false },
