...
async function becomeG3(
  ...
  comptrollerImpl: ComptrollerImpl,
  unitroller: Unitroller,
  qiRate: encodedNumber,
  qiMarkets: string[],
  otherMarkets: string[]
): Promise<World> {
  let invokation = await invoke(
    world,
    comptrollerImpl.methods._become(unitroller._address, qiRate, qiMarkets, otherMarkets),
    from,
    ComptrollerErrorReporter
  );
  ...
  return [
    ...
    new View<{ comptrollerImplArg: StringV; apiKey: StringV }>(
      `
        ...
        * "ComptrollerImpl <Impl> Verify apiKey:<String>" - Verifies Comptroller Implemetation in Avaxscan
    ...
    new Command<{
      ...
      comptrollerImpl: ComptrollerImpl;
      qiRate: NumberV;
      qiMarkets: ArrayV<AddressV>;
      otherMarkets: ArrayV<AddressV>;
    }>(
      `
        #### BecomeG3

        * "ComptrollerImpl <Impl> BecomeG3 <Rate> <QiMarkets> <OtherMarkets>" - Become the comptroller, if possible.
          * E.g. "ComptrollerImpl MyImpl BecomeG3 0.1e18 [qiDAI, qiAVAX, qiUSDC]
      `,
      'BecomeG3',
      [
        new Arg('unitroller', getUnitroller, { implicit: true }),
        new Arg('comptrollerImpl', getComptrollerImpl),
        new Arg('qiRate', getNumberV, { default: new NumberV(1e18) }),
        new Arg('qiMarkets', getArrayV(getAddressV),  {default: new ArrayV([]) }),
        new Arg('otherMarkets', getArrayV(getAddressV), { default: new ArrayV([]) })
      ],
      (world, from, { unitroller, comptrollerImpl, qiRate, qiMarkets, otherMarkets }) => {
        return becomeG3(world, from, comptrollerImpl, unitroller, qiRate.encode(), qiMarkets.val.map(a => a.val), otherMarkets.val.map(a => a.val))
      },
      { namePos: 1 }
    ),
    ...
    new Command<{
      ...
      `
        ...
        * "ComptrollerImpl <Impl> Become <Rate> <QiMarkets> <OtherMarkets>" - Become the comptroller, if possible.
          * E.g. "ComptrollerImpl MyImpl Become 0.1e18 [qiDAI, qiAVAX, qiUSDC]
      `,
      'Become',
      [
        new Arg('unitroller', getUnitroller, { implicit: true }),
