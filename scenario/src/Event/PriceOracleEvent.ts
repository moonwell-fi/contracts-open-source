...
async function setPrice(world: World, from: string, priceOracle: PriceOracle, qiToken: string, amount: NumberV): Promise<World> {
  return addAction(
    world,
    `Set price oracle price for ${qiToken} to ${amount.show()}`,
    await invoke(world, priceOracle.methods.setUnderlyingPrice(qiToken, amount.encode()), from)
  );
}
...
export function priceOracleCommands() {
  return [
    ...
    new Command<{priceOracle: PriceOracle, qiToken: AddressV, amount: NumberV}>(`
        #### SetPrice
      ...
      [
        ...
        new Arg("qiToken", getAddressV),
        new Arg("amount", getExpNumberV)
      ],
      (world, from, {priceOracle, qiToken, amount}) => setPrice(world, from, priceOracle, qiToken.val, amount)
    ),

    new Command<{priceOracle: PriceOracle, address: AddressV, amount: NumberV}>(`
        #### SetDirectPrice

        * "SetDirectPrice <Address> <Amount>" - Sets the per-avax price for the given qiToken
    ...
    new View<{priceOracle: PriceOracle, apiKey: StringV, contractName: StringV}>(`
        ...
        * "Verify apiKey:<String> contractName:<String>=PriceOracle" - Verifies PriceOracle in Avaxscan
