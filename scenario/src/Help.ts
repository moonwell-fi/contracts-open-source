...
export function printHelp(printer: Printer, event: Event, expressions: Expression<any>[], path: string[]=[]) {
  if (event.length === 0) {
    ...
    if (path.length === 0) {
      banner = (
`
## Benqi Command Runner

The Benqi Command Runner makes it easy to interact with Benqi. You can input simple commands
...
sub-commands run \`Help QiToken\` or \`Help QiToken Mint\`.
`).trim();
    } else {
