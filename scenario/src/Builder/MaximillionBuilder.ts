...
export async function buildMaximillion(world: World, from: string, event: Event): Promise<{world: World, maximillion: Maximillion, maximillionData: MaximillionData}> {
  const fetchers = [
    new Fetcher<{qiAvax: AddressV}, MaximillionData>(`
        #### Maximillion
      ...
      async (world, {qiAvax}) => {
        return {
          invokation: await MaximillionContract.deploy<Maximillion>(world, from, [qiAvax.val]),
          description: "Maximillion",
          qiAvaxAddress: qiAvax.val
        };
      },
      {catchall: true}
    )
