...
async function etherBalance(addr: string) {
  return new BigNumber(await web3.eth.getBalance(addr));
}

async function etherGasCost(receipt: any) {
...
function etherExp(num: any) { return etherMantissa(num, 1e18) }
function etherDouble(num: any) { return etherMantissa(num, 1e36) }
function etherMantissa(num: any, scale = 1e18) {
  if (num < 0)
    return new BigNumber(2).pow(256).plus(num);
  return new BigNumber(num).times(scale);
}

function etherUnsigned(num: any) {
  return new BigNumber(num);
}

function mergeInterface(into: any, from: any) {
  const key = (item: any) => item.inputs ? `${item.name}/${item.inputs.length}` : item.name;
  const existing = into.options.jsonInterface.reduce((acc: any, item: any) => {
    acc[key(item)] = true;
    return acc;
  }, {});
  const extended = from.options.jsonInterface.reduce((acc: any, item: any) => {
...
async function freezeTime(seconds: EthersBigNumber) {
  await ethers.provider.send("evm_setNextBlockTimestamp", [seconds.toNumber()]);
}
...
async function sendFallback(contract: any, opts = {}) {
