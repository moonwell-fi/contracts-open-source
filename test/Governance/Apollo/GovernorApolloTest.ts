...
describe('Moonwell Governor Apollo', () => {
  ...
  beforeEach(async () => {
    ...
    const governorFactory = await hre.ethers.getContractFactory("MoonwellGovernorApollo")
  ...
  describe('Governance Parameter Modification', () => {
    ...
    describe('setQuorumCaps', () => {
      it('fails when called externally', async () => {
        // GIVEN a governor
        // WHEN setQuorumCaps is called by an end user
        // THEN the call fails.
        await expect(send(governor, 'setQuorumCaps', [123, 456])).to.be.reverted
      })

      it('fails when quorum caps are invalid', async () => {
        // GIVEN a governor with a successful proposal to adjust quorum caps incorrectly
        // NOTE: Thelower quorum cap is greater than upper quorum cap
        const newLowerCap = 456 
        const newUpperCap = 123
        ...
        await send(
          ...
          [
            ...
            [`${numberToCalldata(newLowerCap)}${numberToCalldata(newUpperCap).slice(2)}`], 
            'UNIT TEST'
          ], 
          { from: alice }
        )
        ...
        // WHEN the proposal is execeuted
        // THEN the call reverts
        await expect(send(governor, 'execute', [proposalID], {from: alice})).to.be.revertedWith("Timelock::executeTransaction: Transaction execution reverted.");
      })

      it('updates when called from timelock', async () => {
        // GIVEN a governor
        // WHEN setQuorumCaps is called by the timelock
        const newLowerCap = 123
        const newUpperCap = 456
        await passProposal(governor.address, 'setQuorumCaps(uint256,uint256)', `${numberToCalldata(newLowerCap)}${numberToCalldata(newUpperCap).slice(2)}`)

        // THEN the quorum caps are updated.
        expect(await call(governor, 'lowerQuorumCap')).to.equal(newLowerCap)
        expect(await call(governor, 'upperQuorumCap')).to.equal(newUpperCap)
      })
    })
  })
  ...
  describe('Proposal Resolution', () => {
    it('quorum achieved without plurality', async () => {
      // GIVEN there is a proposal on the governor
      await send(
        governor, 
        'propose',
        [[governor.address], [0], ['setVotingDelay(uint256)'], [numberToCalldata(123)], 'UNIT TEST'], 
        { from: alice }
      )
      proposalID++      
  ...
  describe('Governance Guardians', () => {
    describe('Guardian Sunset', () => {
      it('sunsetting fails when not performed by timelock', async () => {
        ...
        await expect(send(governor, '__removeGuardians', [])).to.be.revertedWith("GovernorApollo::__removeGuardians: sender must be the timelock")
      })
    ...
    describe('Governance Return Guardian', () => {
      it('governance return address cannot be rotated externally', async () => {
        ...
        // THEN the call is reverted
        await expect(send(governor, '__setGovernanceReturnAddress', [await bob.getAddress()])).to.be.revertedWith("GovernorApollo::__setGovernanceReturnAddress: sender must be gov return guardian")
      })
  ...
  describe('Break Glass Guardian', () => {
    ...
    it('break glass fails when called externally', async () => {
      ...
      // THEN the call reverts
      await expect(send(governor, '__executeBreakGlassOnCompound', [[unitroller.address]])).to.be.revertedWith("GovernorApollo::__breakglass: sender must be bg guardian")  
    })
  })
})
