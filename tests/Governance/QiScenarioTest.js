...
describe('QiScenario', () => {
  ...
  beforeEach(async () => {
    ...
    benqi = await deploy('QiScenario', [root]);
  });
    ...
    [
      ...
      it(`with ${checkpoints} checkpoints, has ${expectedReads} reads`, async () => {
        ...
        while (remaining > 0) {
          ...
          await benqi.methods.generateCheckpoints(amt, offset).send({from: root, gas: 200000000});
          remaining -= amt;
          offset += amt;
        }

        let result = await benqi.methods.getPriorVotes(root, 1).send();
