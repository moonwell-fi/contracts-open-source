...
const oneWeekInSeconds = avaxUnsigned(7 * 24 * 60 * 60);
const zero = avaxUnsigned(0);
...
describe('Timelock', () => {
  ...
  let revertData = encodeParameters(['uint256'], [avaxUnsigned(60 * 60).toFixed()]);
  let eta;
  let queuedTxHash;
  ...
  beforeEach(async () => {
    ...
    blockTimestamp = avaxUnsigned(100);
  ...
  describe('executeTransaction (setPendingAdmin)', () => {
    beforeEach(async () => {
      ...
      delay = avaxUnsigned(configuredDelay);
