...
const { makeComptroller, makeQiToken, makePriceOracle } = require('../Utils/Benqi');
...
describe('ComptrollerV1', function() {
  ...
  describe('delegating to comptroller v1', () => {
    const closeFactor = avaxMantissa(0.051);
    const maxAssets = 10;
    let unitrollerAsComptroller, qiToken;

    beforeEach(async () => {
      unitrollerAsComptroller = await initializeBrains(oracle, avaxMantissa(0.06), 30);
      qiToken = await makeQiToken({ comptroller: unitrollerAsComptroller });
    });
    ...
    describe('_setCollateralFactor', () => {
      ...
      it('fails if not called by admin', async () => {
        expect(
          await send(unitrollerAsComptroller, '_setCollateralFactor', [qiToken._address, half], {
            from: accounts[1]
          })
      ...
      it('fails if factor is too high', async () => {
        const qiToken = await makeQiToken({ supportMarket: true, comptroller: unitrollerAsComptroller });
        expect(
          await send(unitrollerAsComptroller, '_setCollateralFactor', [qiToken._address, one])
      ...
      it('succeeds and sets market', async () => {
        const qiToken = await makeQiToken({ supportMarket: true, comptroller: unitrollerAsComptroller });
        await send(oracle, 'setUnderlyingPrice', [qiToken._address, 1]);
        expect(
          await send(unitrollerAsComptroller, '_setCollateralFactor', [qiToken._address, half])
    ...
    describe('_supportMarket', () => {
      it('fails if not called by admin', async () => {
        expect(
          await send(unitrollerAsComptroller, '_supportMarket', [qiToken._address], { from: accounts[1] })
        ).toHaveTrollFailure('UNAUTHORIZED', 'SUPPORT_MARKET_OWNER_CHECK');
      });

      it('fails if asset is not a QiToken', async () => {
        const notAQiToken = await makePriceOracle();
        await expect(send(unitrollerAsComptroller, '_supportMarket', [notAQiToken._address])).rejects.toRevert();
      });

      it('succeeds and sets market', async () => {
        const result = await send(unitrollerAsComptroller, '_supportMarket', [qiToken._address]);
        expect(result).toHaveLog('MarketListed', { qiToken: qiToken._address });
      });

      it('cannot list a market a second time', async () => {
        const result1 = await send(unitrollerAsComptroller, '_supportMarket', [qiToken._address]);
        const result2 = await send(unitrollerAsComptroller, '_supportMarket', [qiToken._address]);
      ...
      it('can list two different markets', async () => {
        const qiToken1 = await makeQiToken({ comptroller: unitroller });
        const qiToken2 = await makeQiToken({ comptroller: unitroller });
        const result1 = await send(unitrollerAsComptroller, '_supportMarket', [qiToken1._address]);
        const result2 = await send(unitrollerAsComptroller, '_supportMarket', [qiToken2._address]);
