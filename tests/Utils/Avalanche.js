...
const avaxs = require('ethers');

function UInt256Max() {
  return avaxs.constants.MaxUint256;
}
...
function encodeParameters(types, values) {
  const abi = new avaxs.utils.AbiCoder();
  return abi.encode(types, values);
}

async function avaxBalance(addr) {
  return new BigNumber(await web3.eth.getBalance(addr));
}

async function avaxGasCost(receipt) {
...
function avaxExp(num) { return avaxMantissa(num, 1e18) }
function avaxDouble(num) { return avaxMantissa(num, 1e36) }
function avaxMantissa(num, scale = 1e18) {
  if (num < 0)
    return new BigNumber(2).pow(256).plus(num);
  return new BigNumber(num).times(scale);
}

function avaxUnsigned(num) {
...
function keccak256(values) {
  return avaxs.utils.keccak256(values);
}
