...
const timestampsPerYear = 31536000;
...
function baseRoofRateFn(dsr, duty, mkrBase, jump, kink, cash, borrows, reserves) {
  ...
  const stabilityFeePerTimestamp = (duty + mkrBase - 1) * 15;
  const dsrPerTimestamp = (dsr - 1) * 15;
  const gapPerTimestamp = 0.04 / timestampsPerYear;
  const jumpPerTimestamp = jump / timestampsPerYear;

  let baseRatePerTimestamp = dsrPerTimestamp / assumedOneMinusReserveFactor, multiplierPerTimestamp;
  if (baseRatePerTimestamp < stabilityFeePerTimestamp) {
    multiplierPerTimestamp = (stabilityFeePerTimestamp - baseRatePerTimestamp + gapPerTimestamp) / kink;
  } else {
    multiplierPerTimestamp = gapPerTimestamp / kink;
  }

  const ur = utilizationRate(cash, borrows, reserves);

  if (ur <= kink) {
    return ur * multiplierPerTimestamp + baseRatePerTimestamp;
  } else {
    const excessUtil = ur - kink;
    return (excessUtil * jumpPerTimestamp) + (kink * multiplierPerTimestamp) + baseRatePerTimestamp;
  }
}
...
function daiSupplyRate(dsr, duty, mkrBase, jump, kink, cash, borrows, reserves, reserveFactor = 0.1) {
  ...
  const cashSupplyRate = (new BigNum(cash)).times(new BigNum(dsrPerTimestamp)).div(underlying);
  return cashSupplyRate.plus(lendingSupplyRate).toNumber();
}

let fork = "https://kovan-eth.benqi.finance/@14764778";
...
describe('DAIInterestRateModelV3', () => {
  describe("constructor", () => {
    it("sets jug and ilk address and pokes", async () => {
      ...
      let args = [0.5e18, 0.45e18, 500].map(avaxUnsigned);
      // let mult = await call(model, 'multiplierPerTimestamp');
      let sr = await call(model, 'getSupplyRate', [...args, avaxUnsigned(0.1e18)]);
    ...
    [
      ...
      .forEach(([dsr, duty, base, cash, borrows, reserves = 0, jump = 0.8e18, kink = 0.9e18]) => {
        it(`calculates correct borrow value for dsr=${(dsr / 1e25)}%, duty=${(duty / 1e25)}%, base=${(base / 1e25)}%, jump=${jump / 1e18}, cash=${cash}, borrows=${borrows}, reserves=${reserves}`, async () => {
          ...
          const pot = await deploy('MockPot', [ avaxUnsigned(onePlusPerSecondDsr) ]);
          const jug = await deploy('MockJug', [
            avaxUnsigned(onePlusPerSecondDuty),
            avaxUnsigned(perSecondBase)
          ]);
